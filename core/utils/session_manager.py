...
class EnhancedSessionManager:
    ...
    def _initialize_cookies(self) -> bool:
        ...
        try:
            ...
            if self._cookies_ready:
                ...
                logger.warning(
                    "Cookies manager timeout - proceeding with session-only mode")
                return False

        except ImportError:
            logger.warning(
                "streamlit_cookies_manager not available - using session-only mode")
            return False
    ...
    def load_user_session(self) -> bool:
        ...
        if not user_loaded:
            ...
            if user_loaded:
                logger.info(
                    "User session validated from current session state")
                return True
